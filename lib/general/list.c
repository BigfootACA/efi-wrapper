...
list*list_new(mem_pool*pool,void*data){
	errno=ENOMEM;
	struct list*list=mem_allocate(
		pool,sizeof(struct list)
	);
	if(!list)return NULL;
	errno=0;
...
int list_add_new(mem_pool*pool,list*point,void*data){
	list*new=list_new(pool,data);
	if(!new)return -errno;
...
int list_insert_new(mem_pool*pool,list*point,void*data){
	list*new=list_new(pool,data);
	if(!new)return -errno;
...
int list_push_new(mem_pool*pool,list*point,void*data){
	return list_add_new(pool,list_last(point),data);
}
...
int list_unshift_new(mem_pool*pool,list*point,void*data){
	return list_insert_new(pool,list_first(point),data);
}
...
list*list_duplicate(mem_pool*pool,list*lst,list*end){
	if(!lst)EPRET(EINVAL);
	list*x,*r;
	if(
		!(x=list_first(lst))||
		!(r=list_new(pool,x->data))
	)return NULL;
	while((x=x->next)&&x!=end){
		if(list_push_new(pool,r,x->data)<0){
...
list*list_duplicate_chars(mem_pool*pool,list*lst,list*end){
	if(!lst)EPRET(EINVAL);
	list*x,*r;
	if(
		!(x=list_first(lst))||
		!(r=list_new_strdup(pool,x->data))
	)return NULL;
	while((x=x->next)&&x!=end){
		if(list_push_new_strdup(pool,r,x->data)<0){
			list_free_all(r,NULL);
			return NULL;
		}
	}
...
int list_obj_insert(list**lst,list*item){
	list*l;
	if(!item||!lst)ERET(EINVAL);
	if(!*lst)*lst=item;
	else if(list_unshift(*lst,item)!=0)return -errno;
	if((l=list_first(*lst)))*lst=l;
	return 0;
}

int list_obj_add_new(mem_pool*pool,list**lst,void*data){
	if(!lst)ERET(EINVAL);
	list*item=list_new(pool,data);
...
int list_obj_insert_new(mem_pool*pool,list**lst,void*data){
	if(!lst)ERET(EINVAL);
	list*item=list_new(pool,data);
	if(!item)return -errno;
	int r=list_obj_insert(lst,item);
	if(r<0)list_free_item(item,NULL);
	return r;
}

int list_obj_replace(list**lst,list*old,list*new){
	if(!lst||!old||!new)ERET(EINVAL);
	list_remove(new);
	if(*lst==old)*lst=new;
	new->prev=old->prev,new->next=old->next;

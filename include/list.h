...
extern int list_push_new(mem_pool*pool,list*point,void*data);
...
extern int list_unshift_new(mem_pool*pool,list*point,void*data);
...
extern list*list_new(mem_pool*pool,void*data);
...
extern list*list_duplicate(mem_pool*pool,list*lst,list*end);

// src/lib/list.c: duplicate chars list
extern list*list_duplicate_chars(mem_pool*pool,list*lst,list*end);
...
// src/lib/list.c: add item to list end
extern int list_obj_add(list**lst,list*item);

// src/lib/list.c: add item to list head
extern int list_obj_insert(list**lst,list*item);

// src/lib/list.c: new and add item to list end
extern int list_obj_add_new(mem_pool*pool,list**lst,void*data);

// src/lib/list.c: new and add item to list head
extern int list_obj_insert_new(mem_pool*pool,list**lst,void*data);

// src/lib/list.c: strip item from a list
extern int list_obj_strip(list**lst,list*item);

// src/lib/list.c: replace item to another item from a list
extern int list_obj_replace(list**lst,list*old,list*new);
...
#define _DECLARE_NOT_NULL(_name,_base,_ret,_notnull,...)_IN _ret _name(mem_pool*pool,__VA_ARGS__ void*data)_notnull(data,_base)
#define _DECLARE_N_NOT_NULL(_name,_base,...)_DECLARE_NOT_NULL(_name,_base,int,_N_NOTNULL,__VA_ARGS__)
#define _DECLARE_NX_NOT_NULL(_name,_base)_DECLARE_N_NOT_NULL(_name,_base(pool,point,data),list*point,)
...
_DECLARE_N_NOT_NULL(list_obj_add_new_notnull,list_obj_add_new(pool,point,data),list**point,)
_DECLARE_NOT_NULL(list_new_notnull,list_new(pool,data),list*,_P_NOTNULL,)
...
#define _DECLARE_X_DUP(_name,_ret,_arg,_ifs,...) \
	_DECLARE_DUP(_name##_dup,_name##_notnull,_ret,mem_memdup(pool,data,len),_arg,_ifs,mem_pool*pool,__VA_ARGS__ void*data,size_t len)\
	_DECLARE_DUP(_name##_strdup,_name##_notnull,_ret,mem_strdup(pool,data),_arg,_ifs,mem_pool*pool,__VA_ARGS__ const char*data)\
	_DECLARE_DUP(_name##_strndup,_name##_notnull,_ret,mem_strndup(pool,data,len),_arg,_ifs,mem_pool*pool,__VA_ARGS__ const char*data,size_t len)
#define _DECLARE_PX_DUP(_name) _DECLARE_X_DUP(_name,int,(pool,point,dup),ret<0,list*point,)
_DECLARE_X_DUP(list_new,list*,(pool,dup),!ret)
_DECLARE_PX_DUP(list_add_new)
_DECLARE_PX_DUP(list_push_new)
_DECLARE_PX_DUP(list_insert_new)
_DECLARE_PX_DUP(list_unshift_new)
_DECLARE_X_DUP(list_obj_add_new,int,(pool,point,dup),ret<0,list**point,)
